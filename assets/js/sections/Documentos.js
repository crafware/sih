...
$(document).ready(function () {
    ...
    //$('#nota_interconsulta').val($('#nota_interconsulta').attr('data-value').split(',')).select2(); //divide cada seleccinad
    if(document.getElementsByName("accion")[0].defaultValue == 'edit'){ 
    ...
    $('select[name=hf_alta]').click(function (e) {
        if($(this).val()=='Otros'){
            ...
            $('input[name=hf_alta_otros]').attr('required', true);
        }else{
            $('.hf_alta_otros').addClass('hide');
            $('input[name=hf_alta_otros]').removeAttr('required');
            $('input[name=hf_alta_otros]').val('');
        }
    });
    ...
    $('body').on('click','.editar-prescripcion',function(){
      ...
      if(confirm('¿QUIERES MODIFICAR ESTA PRESCRIPCIÓN? '+medicamento)){
          ...
          $('#btn_modificar_prescripcion').attr('data-value-anterior',$(this).attr('data-value-anterior'));
                  ...
                  bootbox.prompt({
                    ...
                    buttons: {
                      ...
                      if(motivo!=null && motivo!=''){
                        ...
                        if($('input[name=hf_id]').val() != undefined || $('input[name=hf_id]').val() != ""){
                          nota_id = $('input[name=hf_id]').val();
                          tipo_nota = "hojafrontalf";
                        }else if($('input[name=notas_id]').val() != undefined || $('input[name=notas_id]').val() != ""){
    ...
    //Mostrar prescripciones por revisar en farmacovigilancia
    $('#acordeon_prescripciones_sin_validar').click(function(){
      var paciente = $('input[name=triage_id]').val();
      var val_accion = 6;
      AccionPanelPrescripcion(val_accion, paciente);
    });
    ...
    $('input[name=check_solicitud_interconsulta]').click(function(){
      if($(this).val() == 0){
        ...
        $('.motivo_interconsulta').attr('required', true);
    ...
    $('.guardar-solicitud-hi-abierto').submit(function (e){    
        ...
        $.ajax({
            ...
            beforeSend: function (xhr) {
                ...
                if(data.accion == '0') {
                    bootbox.alert("No hay ningun Diagnóstico, tiene que ingresar al menos uno");
                    e.preventDefault();
                    bootbox.hideAll();
    ...
    $('#btn_modificar_prescripcion').click(function(){
      ...
      var id_notas = document.getElementsByName("notas_id")[0].value;
      if(observacion.length > 30 && observacion.search(" ") == -1){
        window.alert("1.- Error de edición, inténtenlo otra vez" + observacion);
        return 0;
      }
      if(medicamento_id == '1'){
      ...
      $.ajax({
        ...
        success: function(data, textStatus, jqXHR){
          ...
          console.log(data)
          if(data != null){
            window.alert('2.- Error de edición, inténtenlo otra vez ' + data["observacion"]);
            return 0;
          }
    ...
    $("#add_otro_residente").click (function(e) {
        ...
        if(cont >= 4){
          ...
          '<div class="m-b" id=areaResidentes'+ cont +'>'+
          '<div class="col-sm-3" >'+
    ...
    $("#add_otro_residente2").click (function(e) {
      ...
      if(cont >= 4){
        alert('La nota medica solo acepta maximo 3 medicos resi   dentes');
      }
      else{
        cont += 1;
        $("#medicoResidente").append(
    ...
    $('.notaIndicacciones').submit(function (e) {
        e.preventDefault();
        SendAjax($(this).serialize(),'Sections/Documentos/GuardarNotaIndicacion',function (response) {
            if(response.accion=='1'){
                window.opener.location.reload();
                window.top.close();
                //AbrirDocumentoMultiple(base_url+'Inicio/Documentos/GenerarNotas/'+response.notas_id+'?inputVia='+$('input[name=inputVia]').val(),'NOTAS');
          ...
          for(var x = 0; x < data.length; x++){
            ...
            "<div class='panel panel-default'>"+
              "<div class='panel-heading' >"+
                  ...
                  medicamento + " / <small>Fecha Prescripción: "+data[x].fecha_prescripcion+"</small>"+
                  "</a>"+
              "</div>"+
              "<div id='collapse"+x+"' class='panel-collapse collapse'>"+
                "<div class='panel-body panel_contenido_historial"+data[x].prescripcion_id+"'>"+
                  "<table class='tablaPrescripciones' width='100%'>"+
                    "<thead>"+
                    "<tr>"+
                      "<th>Via</th>"+
                      "<th>Dosis</th>"+
        ...
        data:{
          ...
          for(var x = 0; x < data.length; x++){
            ...
            if(data[x].estado == 1 || data[x].estado == 2){
              ...
              accion_cancelar = "class='glyphicon glyphicon-remove desactivar-prescripcion tip pointer'";

              accion_editar = "class='fa fa-pencil tip pointer editar-prescripcion'";
            ...
            var prescripciones = "<tr >"+
              "<td hidden id='fila_idNota"+data[x].prescripcion_anterior_id+"'  >"+data[x].notas_id+"</td>"+
    ...
    function PrescripcionesSinValidar(folio,estado){

      $.ajax({
        url: base_url+"Sections/Documentos/AjaxPrescripcionesSinValidar",
        type:"GET",
        dataType:"json",
        data:{
          folio: folio,
          ...
          $("#table_prescripciones_sin_validar").empty();
          var d = new Date();
          var fechaActual = d.getDate() + "/" + (d.getMonth()+1) + "/" + d.getFullYear();
          ...
          for(var x = 0; x < data.length; x++){
            ...
            var prescripciones = "<tr >"+
              ...
              "<td style='padding-right: 0px;' id='fila_tiempo"+data[x].prescripcion_id+"'>"+data[x].tiempo+" "+data[x].periodo+"</td>"+
            ...
            //   "</td>"+
            // "</tr>";
            $("#table_prescripciones_sin_validar").append(prescripciones);
          }
        },error: function (e) {
            msj_error_serve(e)
            bootbox.hideAll();
    ...
    function AccionPanelPrescripcion(tipo_accion , paciente){
      ...
      $("#historial_medicamentos_sin_validar").attr('hidden',true);
      ...
      switch (tipo_accion) {
        ...
        case 5:
          ...
          break;
        case 6:
          $("#historial_medicamentos_sin_validar").removeAttr('hidden');
          PrescripcionesSinValidar(paciente,"1");
          break;
        case 7:
          $("#historial_notificaciones").removeAttr('hidden');
