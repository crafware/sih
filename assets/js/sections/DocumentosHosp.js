...
$(document).ready(function () {
    ...
    $('input[name=tipo_interrogatorio][value="'+$('input[name=tipo_interrogatorio]').data('value')+'"]').prop("checked",true);
    // Hoja de Notas parte de riesgo de caida
    $('input[name=apertura_ocular][value="'+$('input[name=apertura_ocular]').data('value')+'"]').prop("checked",true);
    $('input[name=respuesta_motora][value="'+$('input[name=respuesta_motora]').data('value')+'"]').prop("checked",true);
    $('input[name=respuesta_verbal][value="'+$('input[name=respuesta_verbal]').data('value')+'"]').prop("checked",true);
    $('input[name=riesgo_caida][value="'+$('input[name=riesgo_caida]').data('value')+'"]').prop("checked",true);
    $('input[name=estado_salud][value="'+$('input[name=estado_salud]').data('value')+'"]').prop("checked",true);
    ...
    $('#check_form_prescripcion').change(function(){
     ...
     $('#medicosBase').typeahead({
        ...
        afterSelect: function(item) {
                ...
                $("input#id_medico_tratante").val(item.empleado_id);
     ...
     * Carga el contenido del modal para las opciones de glasgow    */
    ...
    $('.sum_glasgow').click(function(){
      ...
      $('input[name=escala_glasgow]').val(total);
    });
    ...
    function importarUltimaNota(folio){
     ...
     function agregarResidente() {
        ...
        var sel = $('#residente_id').find(':selected').val(); //Capturo el Value del Grado Residenete
        var text = $('#residente_id').find(':selected').text();//Capturo el Nombre del grado de Residente- Texto dentro del Select
        ...
        newtr = newtr + '<td class="iResidente" >' + sel + '</td>';
        newtr = newtr + '<td>' + nombre + '</td>';
        newtr = newtr + '<td>' + apellidos +'</td>';
        newtr = newtr + '<td>' + cedula + '</td>';
        newtr = newtr + '<td><button type="button" class="btn btn-danger btn-xs remove-item"><i class="fa fa-times"></i></button></td></tr>';
        
        $('#RSelected').append(newtr); //Agrego el Producto al tbody de la Tabla con el id=ProSelected
        
        RefrescaResidente();//Refresco Productos
            
        $('.remove-item').off().click(function(e) {
            $(this).parent('td').parent('tr').remove(); //En accion elimino el Producto de la Tabla
            if ($('#RSelected tr.item').length == 0)
                $('#RSelected .no-item').slideDown(300); 
            RefrescaResidente();
        });        
       $('.iResidente').off().change(function(e) {
            RefrescaRsidente();
       });
    }
    function RefrescaResidente(){
        var ip = [];
        var i = 0;
        //$('#guardar').attr('disabled','disabled'); //Deshabilito el Boton Guardar
        $('.iResidente').each(function(index, element) {
            i++;
            ip.push({ id_resident : $(this).val() });
        });
