...
class Documentos extends Config{
    ...
    public function DOC43051($Paciente) {
        ...
        /* Servicio Tratante */
        $sql['servicio']= $this->config_mdl->_get_data_condition('um_especialidades' ,array(
            'especialidad_id' => $sql['info43051']['ingreso_servicio']), 'especialidad_nombre'  
        )[0];

        $sql['medico_ingresa']= $this->config_mdl->_get_data_condition('os_empleados' ,array(
            'empleado_id' => $sql['info43051']['ingresa_idmedico']),'empleado_nombre,empleado_apellidos,empleado_matricula'  
        )[0];

        $sql['servicio_ingresa']= $this->config_mdl->_get_data_condition('um_especialidades' ,array(
            'especialidad_id' => $sql['medico_ingresa']['empleado_servicio']),'especialidad_nombre'  
        )[0];
    ...
    public function OrdenLaboratorio($Paciente){
        ...
        $sql['medicoEspecialidad']=$this->config_mdl->_get_data_condition('um_especialidades', array(
                'especialidad_id'   =>  $sql['Medico']['empleado_servicio']))[0];
    ...
    public function GenerarNotaEgreso($Nota) {
        ...
        $sql['PINFO']= $this->config_mdl->sqlGetDataCondition('paciente_info',array(
            'triage_id'=>$sql['notaEgreso']['triage_id'] ))[0];
    ...
    public function GenerarNotaProcedimientos($Nota) {
        ...
        $sql['PINFO']= $this->config_mdl->sqlGetDataCondition('paciente_info',array(
            'triage_id'=>$sql['notaProcedimientos']['triage_id'] ))[0];
    ...
    public function GenerarNotaIndicaciones($Nota) {
       ...
       /* $sql['infoCama'] = $this->config_mdl->_query("SELECT * FROM os_pisos, os_pisos_camas, os_camas WHERE 
        ...
        $sql['PINFO']= $this->config_mdl->sqlGetDataCondition('paciente_info',array(
            'triage_id'=>$sql['NotaIndicacion']['triage_id'] ))[0];
