...
<<<<<<< HEAD
  ...
  <page_footer>
    ...
    ?>
    <div style="position: absolute;top: <?= $top ?>px;left: 215px;width: 540px;font-size: 10px">
      <b>Dr. <?= $medicoTratante['empleado_apellidos'] ?> <?= $medicoTratante['empleado_nombre'] ?> MATRICULA: <?= $medicoTratante['empleado_matricula'] ?></b>
    </div>
    <span style="margin-top: 10px;"><b>MÉDICOS RESIDENTES</b></span>
    <?php if(!empty($residentes)) {?>    
      <div style="position: absolute;top: <?=$top+40?>px;left: 260px;width: 500px;font-size: 10px;text-align: left;">
        <span style="margin-top: 0px;"><b>MÉDICOS RESIDENTES</b></span>  
        <?php foreach ($residentes as $value){?>
          <b>Dr. <?=$value['apellido_residente']?>  <?=$value['nombre_residente']?> MATRICULA: <?=$value['cedulap_residente']?> GRADO: <?=$value['grado']?></b><br>   
        <?php }?>  
      </div>
    <?php }?>
  </page_footer>
