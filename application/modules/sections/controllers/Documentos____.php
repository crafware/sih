...
class Documentos extends Config{
    ...
    public function AjaxCambiarEstadoPrescripcion(){
      ...
      if( $this->input->get('tipo_nota') == 'hojafrontalf' ){
        $this->config_mdl->_delete_data('nm_hojafrontal_prescripcion',
                                         array('hf_id' => $this->input->get('nota_id'),
                                               'prescripcion_id' => $this->input->get('prescripcion_id')));
      }else if( $this->input->get('tipo_nota') == 'evolucion' ){
        $this->config_mdl->_delete_data('nm_notas_prescripcion',
                                         array('notas_id' => $this->input->get('nota_id'),
                                               'prescripcion_id' => $this->input->get('prescripcion_id')));
      }
    ...
    public function AjaxHojaInicialAbierto() {
      ...
      foreach ($this->input->post('procedimientos') as $procedimientos_select) {
        ...
        $this->config_mdl->_update_data('paciente_info',  array('alergias' => $alergias),
    ...
    public function Notas($Nota) {
        ...
        $sql['Medicos']= $this->config_mdl->_query("SELECT * FROM os_empleados, os_empleados_roles, os_roles WHERE
                                                    ...
                                                    os_roles.rol_id=2");
        $sql['medicosResidentes']= $this->config_mdl->_query("SELECT * FROM os_empleados, os_empleados_roles, os_roles WHERE
                                                    os_empleados_roles.empleado_id=os_empleados.empleado_id AND
                                                    os_empleados_roles.rol_id=os_roles.rol_id AND
                                                    os_roles.rol_id=77");
        $sql['MedicosBase']= $this->config_mdl->_query("SELECT empleado_id, empleado_matricula,empleado_nombre, empleado_apellidos FROM os_empleados
        ...
        foreach($sql['Residentes'] as $valor) {}        
        //---------------laboratorio----------///
        
        if($_GET['TipoNota']=='Nota de Egreso') {
            $sql['NotaEgreso']= $this->config_mdl->_query("SELECT * FROM doc_notas, doc_nota_egreso, um_especialidades WHERE
            ...
            $sql['NotaPreAlta']= $this->config_mdl->_query("SELECT
                            *
                            FROM
                                doc_notas
                            ...
                            doc_nota_egreso.docnota_id = um_alta_hospitalaria.id_nota_egreso AND
                                ...
                                doc_notas.notas_id =".$Nota)[0];
            $sql['Residentes'] = $this->config_mdl->sqlGetDataCondition('um_notas_residentes',array(
                'notas_id'=> $sql['NotaPreAlta']['nota_id']
                ));
            
            $this->load->view('Documentos/Nota_Header',$sql);
            $this->load->view('Documentos/Nota_Prealta',$sql);
    ...
    public function AjaxNotaProcedimientos () {   
        ...
        if($this->input->post('accion')=='add'){
            ...
            $this->config_mdl->_update_data('um_notas_procedimientos',array(
                'resumen_procedimiento' => $this->input->post('resumenProcedimiento'),
                'procedimientos'        => trim($procedimientos, ',')),array(
                'notas_id'              =>  $notas_id
            ));
