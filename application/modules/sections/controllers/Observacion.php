...
class Observacion extends MX_Controller{
    ...
    public function AjaxCamas() {
        $sql= $this->config_mdl->_query("SELECT * FROM os_camas, os_areas
            WHERE os_camas.area_id=os_areas.area_id AND os_camas.cama_status='Ocupado' AND os_areas.area_id=".$this->input->get_post('area')." ORDER BY os_camas.cama_ingreso_f ASC, os_camas.cama_ingreso_h DESC");
