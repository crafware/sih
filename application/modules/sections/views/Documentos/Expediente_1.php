                    ...
                    <li class="">
                      ...
                      <?php }else {?>
                        ...
                        <li class="<?=!empty($NotaIngresoHospital)  ? 'disabled' :''?>">
                            <a <?php if(empty($NotaIngresoHospital)){?>href="<?= base_url()?>Sections/Documentos/NotaIngresoHospitalario?idnota=0&TipoNota=Nota Inicial&a=add&folio=<?=$this->uri->segment(4)?>&via=<?=$this->UMAE_AREA?>&tipo=<?=$_GET['tipo']?>" target="_blank" rel="opener" <?php }?>><b>Generar "Nota de Ingreso"</b></a>
                        </li>
                        <?php }?>
                        ...
                        <li class="">
                            <a href="<?= base_url()?>Sections/Documentos/Notas/0/?a=add&TipoNota=Nota de Evolución&folio=<?=$this->uri->segment(4)?>&via=Covid&doc_id=<?=$_GET['doc_id']?>&inputVia=<?=$_GET['tipo']?>" target="_blank" rel="opener" ><b>Generar "Nota de Evolución"</b></a>
                        </li> 
                        <li class="">
                            <a href="<?= base_url()?>Sections/Documentos/Notas/0/?a=add&TipoNota=Nota de Egreso&folio=<?=$this->uri->segment(4)?>&via=Covid&doc_id=<?=$_GET['doc_id']?>&inputVia=<?=$_GET['tipo']?>" target="_blank" rel="opener" ><b>Generar "Nota de Egreso"</b></a>
                        </li>                                        
                        <?php }else {?>
                        ...
                        <li class="<?=empty($NotaIngresoHospital)  ? 'disabled' :''?>">
                            <a <?php if(!empty($NotaIngresoHospital)){?>href="<?= base_url()?>Sections/Documentos/Notas/0/?a=add&TipoNota=Nota de Prealta&folio=<?=$this->uri->segment(4)?>&via=<?=$_GET['via']?>&doc_id=<?=$_GET['doc_id']?>&inputVia=<?=$_GET['tipo']?>" target="_blank" rel="opener" <?php }?>><b>Generar "Nota de Pre-alta"</b></a>
                        </li>
                        <?php }?>
            ...
            <div class="col-md-12" style="margin-top: 0px">
              ...
              <div class="panel with-nav-tabs">
                <div class="panel-heading">
                  ...
                  <ul class="nav nav-tabs table-hover grey-400">
                    <li class="active bold"><a href="#tab1primary" id="btnNotasTriage" data-toggle="tab">Notas Médicas</a></li>
                    <li class="font-weight-bold"><a href="#tab2primary" data-toggle="tab">Notas de Trabajo Social</a></li>
                ...
                <div class="panel-body">
                  <div class="tab-content">
                                ...
                                <tr>
                                    ...
                                    <td> <!-- Acciones -->
                                        ...
                                        <?php if($value['empleado_id']==$_SESSION['UMAE_USER']){?>
                                            <a style="display:none;" id = <?=$value['notas_id']."EditNotas_id"?> href="<?=  base_url()?>Sections/Documentos/Notas/<?=$value['notas_id']?>/?a=edit&TipoNota=<?=$value['notas_tipo']?>&folio=<?=$this->uri->segment(4)?>&via=<?=$value['notas_via']?>&doc_id=<?=$_GET['doc_id']?>&inputVia=<?=$_GET['tipo']?>" target="_blank" rel="opener">
                                              <i  class="fa fa-pencil icono-accion tip pointer" data-original-title="Editar Nota"></i>
                                            </a>&nbsp;
                    ...
                    <!-- 
                      Tab de prescripciones medicas 
                    -->
                    <div class="tab-pane fade" id="tab5primary">
                      <div class="row">
                        <div class="col-md-12">
                          <button id="btnAdd" type="button" class="btn btn-info tip" data-original-title="Agregar Medicamento" data-placement="right" onclick="window.open('<?= base_url()?>Sections/Prescripcion/Agregar/<?=$this->uri->segment(4)?>', '_blank')">
                      ...
                      <div class="row">
                        <div class="col-md-12" style="margin-top: 10px;">
                          <?php $cont=1; foreach ($prescripciones_fecha as $value) {?>
                            <div class="accordion" id="accordion2">
                              <div class="accordion-group">
                                <!-- <span class="pull-right clickable"><i class="glyphicon glyphicon-chevron-up"></i></span> -->
                                <div class="accordion-heading">
                                  <div class="col-md-12">
                                  <a class="accordion-toggle link-acordeon" data-toggle="collapse" data-parent="#accordion2" href="#collapse<?php echo $cont?>">
                                    <div class="col-md-3">
                                      <i class="fa fa-calendar"></i> <span><?=$value['fecha']?></span>
                                    </div>
                                    <div class="col-md-4">
                                      <i class="fa fa-user-md"></i> <span><?=$value['empleado_apellidos']?> <?=$value['empleado_nombre']?></span>                  
                                    </div>
                                    <div class="col-md-3">
                                      <i class="fa fa-medkit"></i> <span><?=$value['especialidad_nombre']?></span>
                                    </div>
                                  </a>
                                  </div>
                                </div>
                                <div id="collapse<?php echo $cont?>" class="accordion-body collapse">
                      ...
                      <div class="row">
                        <div class="col-md-12" style="margin-top: 10px;"> 
                          <div class="panel-heading" style="background-color: #e3f2fd">
                            <ul class="nav nav-tabs table-hover submenu">
                              <li class="active">
                                <a class="collapse showclass submenu_prescripciones" id="acordeon_prescripciones_activas" data-toggle="tab">Activas:
                                    <label class="indicador" id="label_total_activas"><?= $Prescripciones_activas[0]['activas'] ?></label>
                                </a>
                              </li>
                              <li>
                                <a class="submenu_prescripciones" id="acordeon_prescripciones_pendientes" data-toggle="tab">Por validar:
                                    <label class="indicador" id="label_total_pendientes"><?= $Prescripciones_pendientes[0]['pendientes'] ?></label>
                                </a>
                              </li>                          
                              <li>
                                <a class="submenu_prescripciones" id="acordeon_prescripciones_canceladas" data-toggle="tab">Canceladas o actualizadas:
                                    <label class="indicador" id="label_total_canceladas"><?= count($Prescripciones_canceladas) ?></label>
                                </a>
                              </li>
                              <li>
                                <a class="submenu_prescripciones" id="acordeon_reacciones" data-toggle="tab">Reacciones adversas:
                                    <label class="indicador" id="label_total_reacciones"><?= count($ReaccionesAdversas) ?></label>
                                </a>
                              </li>
                              <li>
                                <a class="submenu_prescripciones" id="acordeon_notificaciones" data-toggle="tab">Interacciones farmacologica:
                                    <label class="indicador" id="label_total_notificaciones"><?= count($Notificaciones) ?></label>
                                </a>
                              </li>
                              <li>
                                <a class="submenu_prescripciones" id="acordeon_alergia_medicamentos" data-toggle="tab">Alergia a medicamentos:
                                    <label class="indicador" id="label_total_reacciones" ><?= count($AlergiaMedicamentos) ?></label>
                                </a>
                              </li>  
                              <li>
                                <a class="submenu_prescripciones" id="acordeon_notificaciones" data-toggle="tab">Notificaciones Farmacovigilancia:
                        ...
                        <div>
                          <table class="table table-bordered table-hover footable table-no-padding" id="historial_medicamentos_activos" style="width:100%;" data-filter="#filter">
...
console.log(HojasFrontales);
console.log(NotaIngresoHospital);*/
...
var x = setInterval(function() {
  ...
  for(var i = 0;i<NotasAllTime.length; i++){
    ...
    if (distance < 0) {
      document.getElementById(NotasAll[i]['notas_id']+"expiredTimeNotas_id").innerHTML = uneditableMessage;
      EditNotas = document.getElementById(NotasAll[i]['notas_id']+"EditNotas_id")
      if(EditNotas){
        EditNotas.remove();
      }
    }else{
      document.getElementById(NotasAll[i]['notas_id']+"expiredTimeNotas_id").innerHTML = days + "d " + hours + "h "+ minutes + "m " + seconds + "s ";
    }
  }
  for(var i = 0;i<HojasFrontalesTime.length; i++){
  // Find the distance between now and the count down date
    ...
    var seconds = Math.floor((distance % (1000 * 60)) / 1000);
    if (distance < 0) {
      document.getElementById(HojasFrontales[i]['hf_id']+"expiredTimeHf_id").innerHTML = uneditableMessage;
      EditNotas = document.getElementById(HojasFrontales[i]['hf_id']+"EditHf_id")
      if(EditNotas){
        EditNotas.remove();
      }
    }else{
      document.getElementById(HojasFrontales[i]['hf_id']+"expiredTimeHf_id").innerHTML = days + "d " + hours + "h "+ minutes + "m " + seconds + "s ";
    }
  }
  for(var i = 0;i<NotaIngresoHospitalTime.length; i++){
    ...
    var seconds = Math.floor((distance % (1000 * 60)) / 1000);
    if (distance < 0) {
      document.getElementById(NotaIngresoHospital[i]['id_nota']+"expiredTimeId_nota").innerHTML = uneditableMessage;
      EditNotas = document.getElementById(NotaIngresoHospital[i]['id_nota']+"EditId_nota")
      if(EditNotas){
        EditNotas.remove();
      }
    }else{
      document.getElementById(NotaIngresoHospital[i]['id_nota']+"expiredTimeId_nota").innerHTML = days + "d " + hours + "h "+ minutes + "m " + seconds + "s ";
    }
  }
}, 1000);
</script>