...
class MinimaInvacion extends Config{ 
    ...
    public function ProcedimientoQuirofano() {
        $select['PROCEDIMIENTOS'] = $this->config_mdl->_query("SELECT *FROM abs_vale_servicio, os_triage, paciente_info, abs_procedimiento 
                                                                WHERE abs_vale_servicio.triage_id = os_triage.triage_id 
                                                                AND os_triage.triage_id = paciente_info.triage_id
                                                                AND abs_procedimiento.procedimiento_codigo = abs_vale_servicio.procedimiento_codigo
    ...
    public function ValeServicio() {
        ...
        if(isset($_GET['servicio'])){
            $sql['servicio']= $this->config_mdl->_query("SELECT *FROM os_observacion_ci 
                                                         WHERE os_observacion_ci.ci_id =".$_GET['servicio'])[0];
        }else{
            $sql['servicio'] ='';
        }
    ...
    public function GuardarValeServicio() {
        ...
        if($this->input->post('accion') == 'add'){
            ...
            $this->config_mdl->_insert('abs_vale_servicio', array(
                ...
                'matricula_medica'=> $this->input->post('matricula_medica'),
    ...
    public function AjaxBuscarPaciente() {
        $sql = $this->config_mdl->_query("SELECT * FROM os_triage, paciente_info 
                                          WHERE os_triage.triage_id=paciente_info.triage_id 
                                          AND os_triage.triage_id =".$this->input->post('triage_id'))[0];
        $action = $this->input->post('action');
        if(!empty($sql)){
            $this->setOutput(array('accion'=>'1', 'action'=>$action));
        }else{
            $this->setOutput(array('accion'=>'2'));
        }
    }
    ...
    public function AjaxBuscarMedico() {
        $sql= $this->config_mdl->_query("SELECT os_empleados.empleado_apellidos, os_empleados.empleado_nombre 
                                                    FROM os_empleados 
                                                    WHERE os_empleados.empleado_matricula =".$this->input->post('medico_matricula'))[0];
    ...
    public function AjaxBuscarSolicitud() {
        $sql= $this->config_mdl->_query("SELECT *FROM os_observacion_ci 
                                         WHERE os_observacion_ci.ci_id =".$this->input->post('solicitud'))[0];
