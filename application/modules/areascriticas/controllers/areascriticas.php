...
class Areascriticas extends Config
{
    ...
    public function AjaxGetPacienteUCI()
    {
        ...
        if (!empty($sqlConsultorio)) {
            $sqlPacienteUCI = $this->config_mdl->_query("SELECT * FROM um_pacientes_".$area." WHERE 
                                                    triage_id =" . $this->input->post('triage_id') . " AND fecha_egreso_".$area." IS NULL");
            if (empty($sqlPacienteUCI)) {
                $this->config_mdl->_insert('um_pacientes_'.$area, array(
                    'id_doc43051'       => $sqlConsultorio[0]["id"],
                    "triage_id"         => $sqlConsultorio[0]["triage_id"],
                    "fecha_ingreso_".$area => date("Y-m-d H:i")
                ));
