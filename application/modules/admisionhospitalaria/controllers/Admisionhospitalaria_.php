...
class Admisionhospitalaria extends Config{
  ...
  public function AjaxVisorCamas() {
    ...
    foreach ($Pisos as $value) {
        ...
        //$Limpieza= $this->TotalCamasEstatusPisos($value['piso_id'], 'En Limpieza');   // Esta sucia
        //$Limpias = $this->TotalCamasEstatusPisos($value['piso_id'], 'Limpia');         // Esta Limpia
        //$Mantenimiento= $this->TotalCamasEstatusPisos($value['piso_id'], 'En Mantenimiento'); // Esta descompuesta
        //$Asignado= $this->TotalCamasEstatusPisos($value['piso_id'], 'Asignado');       // Esta en espera
        //$Contaminada=$this->TotalCamasEstatusPisos($value['piso_id'], 'Contaminada');
        $TotalPiso=$this->TotalPorPiso($value['piso_id']);
        $PisoMombreCorto = str_replace(str_split(' '), '-', $value['piso_nombre_corto']);
        $Col.='<div class="container-fluid col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="bedCharts-space text-left">
                        ...
                        <span class="infoPisoOcupadas">Ocupadas: '.$Ocupadas.'</span>
                    </div>  
        ...
        foreach ($Camas as $valor) { 
            ...
            $dataTitle='';
            //$NombrePaciente='';
            //$Triage=$this->config_mdl->_get_data_condition("os_triage",array('triage_id'=>$valor['triage_id']))[0];
            //$PacienteInfo = $this->config_mdl->_get_data_condition("paciente_info",array('triage_id'=>$valor['triage_id']))[0];
            $info43051=$this->config_mdl->_get_data_condition('doc_43051',array('triage_id'=>$valor['triage_id']))[0];
            //$InfoRegistro=$this->config_mdl->_get_data_condition("os_asistentesmedicas",array('triage_id'=>$valor['triage_id']))[0];
            
            /*$sqlMedico= $this->config_mdl->_get_data_condition('os_empleados',array(
                'empleado_id'=> $info43051['ingreso_medico']
                ),'empleado_nombre, empleado_apellidos, empleado_matricula')[0];*/
            //$Medico = $sqlMedico['empleado_apellidos'].' '.$sqlMedico['empleado_nombre'];

            /*$Servicio= $this->config_mdl->_get_data_condition('um_especialidades',array(
                'especialidad_id'=>$info43051['ingreso_servicio']))[0];*/
            /*$Asigna=$this->config_mdl->sqlGetDataCondition("os_empleados",array(
                'empleado_id'=>$info43051['empleado_asigna']),
                'empleado_id,empleado_nombre,empleado_apellidos')[0];*/
            //$asignaNombre=$Asigna['empleado_nombre'].' '.$Asigna['empleado_apellidos'];
            //$fechaRegistro=$InfoRegistro['asistentesmedicas_fecha'];
           
            //$NombrePaciente=$Triage['triage_nombre'].' '.$Triage['triage_nombre_ap'].' '.$Triage['triage_nombre_am'];
            //$nss=$PacienteInfo['pum_nss'].'-'.$PacienteInfo['pum_nss_agregado'];
            ...
            $Imprimir43051='<li><a href="#" class="generar43051" data-triage="'.$info43051['triage_id'].'" data-cama="'.$valor['cama_id'].'"><i class="fa fa-print icono-accion"></i> Generar 43051</a></li>';
            ...
            $LiberarCama='<li><a href="#" class="liberar43051" data-triage="'.$info43051['triage_id'].'" data-cama="'.$value['cama_id'].'"><i class="fa fa-share-square-o icono-accion"></i> Liberar Cama</a></li>'; 
            ...
            if($valor['cama_estado']=='Disponible'){ //vestida-color verde
                ...
                if($this->UMAE_AREA === 'Admisión Hospitalaria') {
                    $acciones = '<ul class="list-inline list-menu">
                                    <li class="dropdown">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-scale pull-left pull-up" style="margin-left: -5px">'.$nombreCama.' '.$CambiarCama.' '.$Imprimir43051.' '.$CancelarIngreso.' '.$LiberarCama.' '.$AltaPaciente.'</ul>
                                    </li>
                                </ul>';
                ...
                $acciones = '<ul class="list-inline list-menu">
                                    <li class="dropdown">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-scale pull-left pull-up" style="margin-left: -5px">'.$nombreCama.' '.$CambiarCama.' '.$Imprimir43051.' '.$CancelarIngreso.' '.$LiberarCama.' </ul>
                                    </li>
                                </ul>';
            ...
            /*$Col.='<div class="fila '.$borde.'">
                    <div style="color: '.$color.';"><strong><center>'.$proceso.'</center></strong></div>
                    <div rel="tooltip" class="cama-no cama-celda '.$CamaStatus.' color-white cama'.$valor['cama_id'].'" "  data-toggle="tooltip" data-animation="true" role="checkbox" data-cama="'.$valor['cama_id'].'" data-estado="'.$valor['cama_estado'].'" data-cama_nombre="'.$valor['cama_nombre'].'" data-folio="'.$valor['triage_id'].'" data-paciente="'.$proceso.'" data-toggle="tooltip" data-trigger="hover"
            ...
            /*$Col.='<div class="contenedor fila '.$borde.'">
                <div style="color: '.$color.';"><strong><center>'.$proceso.'</center></strong></div>
                    <div id="'.$valor['cama_id']."_".$PisoMombreCorto.'" rel="tooltip" class="cama-no cama-celda '.$CamaStatus.' color-white cama'.$valor['cama_id'].'" "  data-toggle="tooltip" data-animation="true" role="checkbox" data-cama="'.$valor['cama_id'].'" data-estado="'.$valor['cama_estado'].'" data-cama_nombre="'.$valor['cama_nombre'].'" data-folio="'.$valor['triage_id'].'" data-paciente="'.$proceso.'" data-toggle="tooltip" data-trigger="hover"
                        data-placement="top" data-html="true">
                        '.$acciones.'
                        <h6 style="margin-top: 3px; color:black"><b>'.$valor['cama_id'].'</b></h6>
                            <div class="tooltip" id="tooltip'.$valor['cama_id']."_".$PisoMombreCorto.'">
                                <div class="thumb">
                                    <img src="./img/torre-eiffel.png" alt="">
                                </div>
                                <div class="info">
     ...
     public function AjaxConfirmarIngreso() {
         ...
         $data=array(
            ...
            $this->config_mdl->_insert('os_areas_pacientes', array(
                ...
                'cama_id'=> $this->input->post('cama_id'),
                'triage_id'=> $this->input->post('triage_id'),
                'idmedico_tratante'=> $sql43051[0]['ac_ingreso_medico'],
    ...
    public function ProcesoDeAlta() {
        ...
        // $infoP = $this->config_mdl->_get_data_condition('os_triage', array(
        //      'triage_id' => $infoC['triage_id']
        //  ))[0];
        ...
        if(!empty($notaEgreso)) {
            switch ($notaEgreso['motivo_egreso']) {
               ...
               $tiempoEstancia=Modules::run('Config/CalcularTiempoTranscurrido',array(
                     'Tiempo1' =>  str_replace('/', '-',$infoP['fecha_ingreso']).' '.$infoP['hora_ingreso'],
                ...
                $this->setOutput(array('accion'      => '1',
                                       ...
                                       'testancia'   => $tiempoEstancia->d.' dias '.$tiempoEstancia->h.' hrs' ));
    ...
    public function ConfirmarAltaCamaAsistenteMedica(){
        ...
        $this->config_mdl->_update_data('doc_43051',array(
                ...
                'salida_servicio' => $notaAlta['empleado_servicio_id'],
                'salida_medico'   => $notaAlta['notas_medicotratante'],
        ...
        $this->config_mdl->_insert('um_reporte_egresos_hospital',array(
                ...
                'idservicio'            => $notaAlta['especialidad_id'], 
                'iddxegreso'            => $dx['diagnostico_id'],          
                'cama'                  => $paciente['cama_nombre'].' '.$paciente['piso_nombre_corto'],
                'medico'                => $notaAlta['empleado_apellidos'].' '.$notaAlta['empleado_nombre'],
                'servicio'              => $notaAlta['especialidad_nombre'],
                ...
                'fecha_prealta'         => $paciente['fecha_nota'],         
                'fecha_h_prealta'       => $notaAlta['fecha_nota'].' '.$notaAlta['hora_nota'],
                'fecha_h_alta_medico'   => $notaAlta['fecha_hora_alta'], // fecha y hora en que medico da de alta 
