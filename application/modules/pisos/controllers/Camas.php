...
class Camas extends Config{
    ...
    public function AjaxGestionCamas() {
        if($this->input->post('area')==''){
            if($this->UMAE_AREA=='Urgencias' || $this->UMAE_AREA=='Admisión Hospitalaria' || $this->UMAE_AREA=='Jefa Asistentes Médicas'){
                $Condition="AND os_areas.area_id IN(3,4,5,6)";
            }else{
                $Condition="AND os_areas.area_id>6";
            }
            $Mantenimiento= $this->config_mdl->_query("SELECT COUNT(os_camas.cama_id) AS Mantenimiento FROM os_camas, os_areas
                WHERE os_camas.area_id=os_areas.area_id AND os_camas.cama_estado='En Mantenimiento' $Condition")[0]['Mantenimiento'];
                $Limpieza= $this->config_mdl->_query("SELECT COUNT(os_camas.cama_id) AS Limpieza FROM os_camas, os_areas
                WHERE os_camas.area_id=os_areas.area_id AND os_camas.cama_estado='En Limpieza' $Condition")[0]['Limpieza'];
                $Ocupadas= $this->config_mdl->_query("SELECT COUNT(os_camas.cama_id) AS ocupadas FROM os_camas, os_areas
                WHERE os_camas.area_id=os_areas.area_id AND os_camas.cama_estado='Ocupado' $Condition")[0]['ocupadas'];
                $Disponible= $this->config_mdl->_query("SELECT COUNT(os_camas.cama_id) AS disponible FROM os_camas, os_areas
                WHERE os_camas.area_id=os_areas.area_id AND os_camas.cama_estado='Disponible' $Condition")[0]['disponible'];
                $Total= $this->config_mdl->_query("SELECT COUNT(os_camas.cama_id) AS total FROM os_camas, os_areas
                WHERE os_camas.area_id=os_areas.area_id $Condition")[0]['total'];
        }else{
            $Mantenimiento= $this->config_mdl->_query("SELECT COUNT(os_camas.cama_id) AS Mantenimiento FROM os_camas, os_areas
            WHERE os_camas.area_id=os_areas.area_id AND os_camas.cama_estado='En Mantenimiento' AND os_areas.area_id=".$this->input->post('area'))[0]['Mantenimiento'];
            $Limpieza= $this->config_mdl->_query("SELECT COUNT(os_camas.cama_id) AS Limpieza FROM os_camas, os_areas
            WHERE os_camas.area_id=os_areas.area_id AND os_camas.cama_estado='En Limpieza' AND os_areas.area_id=".$this->input->post('area'))[0]['Limpieza'];
            $Ocupadas= $this->config_mdl->_query("SELECT COUNT(os_camas.cama_id) AS ocupadas FROM os_camas, os_areas
                    WHERE os_camas.area_id=os_areas.area_id AND os_camas.cama_estado='Ocupado' AND os_areas.area_id=".$this->input->post('area'))[0]['ocupadas'];
            $Disponible= $this->config_mdl->_query("SELECT COUNT(os_camas.cama_id) AS disponible FROM os_camas, os_areas
                    WHERE os_camas.area_id=os_areas.area_id AND os_camas.cama_estado='Disponible' AND os_areas.area_id=".$this->input->post('area'))[0]['disponible'];
            $Total= $this->config_mdl->_query("SELECT COUNT(os_camas.cama_id) AS total FROM os_camas, os_areas
    ...
    public function CamasDetalles() {
        if($this->input->get('tipo')=='Total'){
            ...
            $sql['Gestion']= $this->config_mdl->_query("SELECT * FROM os_camas, os_areas
                    WHERE os_camas.area_id=os_areas.area_id AND os_camas.cama_estado='Disponible'  AND os_areas.area_id=".$this->input->get('area'));
        }if($this->input->get('tipo')=='Ocupados'){
            $sql['Gestion']= $this->config_mdl->_query("SELECT * FROM os_camas, os_areas
                    WHERE os_camas.area_id=os_areas.area_id AND os_camas.cama_estado='Ocupado'  AND os_areas.area_id=".$this->input->get('area')." ORDER BY cama_ingreso_f ASC, cama_ingreso_h ASC");
        }if($this->input->get('tipo')=='Mantenimiento'){
            $sql['Gestion']= $this->config_mdl->_query("SELECT * FROM os_camas, os_areas
                    WHERE os_camas.area_id=os_areas.area_id AND os_camas.cama_estado='En Mantenimiento'  AND os_areas.area_id=".$this->input->get('area'));
        }if($this->input->get('tipo')=='Limpieza'){
            $sql['Gestion']= $this->config_mdl->_query("SELECT * FROM os_camas, os_areas
                    WHERE os_camas.area_id=os_areas.area_id AND os_camas.cama_estado='En Limpieza'  AND os_areas.area_id=".$this->input->get('area'));
        }
    ...
    public function AjaxLimpiezaCamas() {
        if($this->UMAE_AREA=='Administrador' || $this->UMAE_AREA=='Limpieza Camas'){
    ...
    public function AjaxVisorCamas() {
        ...
        $TOTAL_CD= count($this->config_mdl->_get_data_condition('os_camas',array('cama_estado'=>'Disponible')));
        $TOTAL_CO= count($this->config_mdl->_get_data_condition('os_camas',array('cama_estado'=>'Ocupado')));
        $TOTAL_CM= count($this->config_mdl->_get_data_condition('os_camas',array('cama_estado'=>'En Mantenimiento')));
        $TOTAL_CL= count($this->config_mdl->_get_data_condition('os_camas',array('cama_estado'=>'En Limpieza')));
        $TOTAL_CDES= count($this->config_mdl->_get_data_condition('os_camas',array('cama_estado'=>'En Espera')));
        $Camas=  $this->config_mdl->_query("SELECT * FROM os_camas, os_areas
        ...
        if(!empty($Camas)){
            foreach ($Camas as $value) {
                ...
                }else if($value['cama_estado']=='En Espera'){
                    $AccionCambioE='hidden';
                    $CamaStatus='blue-grey-700';                    
                    $Paciente=$this->config_mdl->_get_data_condition('os_triage',array(
                        'triage_id'=> $value['cama_dh']
    ...
    public function LogPisos($data) {
        $this->config_mdl->_insert('um_pisos_log',array(
